...
<!-- HANDOFF:START -->
## 🔄 Handoff — 2026-06-22 (Transferência rica PLANEJADA — pronta para codar)

**Foco da próxima sessão:** **implementar a Fase 1** da feature "Nó Transferência rico" (sonda + camada de dados de vendedores), seguindo **PLANS.md § "Feature — Nó Transferência rico"** (1ª seção do arquivo). Plano fechado por interrogatório nesta sessão; nada codado ainda. *(Obs.: a feature "Chamada de API" também está planejada e pronta para codar, na 2ª seção — decidir a ordem; só uma das duas pega o próximo bump.)*

**Onde paramos:** branch `feat/execution-delay`, versão **0.23.0**. Esta sessão **só planejou** (interrogatório do nó Transferência) — adicionou a seção de plano no topo do [PLANS.md](PLANS.md). **Nenhum código de feature.** Working tree segue com os untracked de antes (logos `.png/.svg`, `masterFlow.json`, `scripts/probe-*.mjs`, `src/utils/endpoints*.ts`) — não mexidos aqui.

**Fios soltos / meio-feito:**
- **Tudo a codar** (Fase 1 primeiro) — ver mapa no PLANS: sonda `probe-users.mjs` → `users.ts`+testes → fiação no contexto (`users`/`usersStatus`/`loadUsers`/`usersById`). Fase 2 = editor 2 níveis + gate + release.
- **⚠️ Base da API NÃO confirmada:** o curl do Andy para `getSupervisedUsersV2` usa `api-private.omni.chat` (SEM "2"), mas o projeto usa `PARSE = api-private2.omni.chat/parse` ([teams.ts:21](src/utils/teams.ts#L21)). **A sonda da Fase 1 decide isso antes de escrever `users.ts`.** Shape do usuário (`objectId`+nome+email?) idem.
- **Risco >100 vendedores:** busca filtra no cliente com `limit:100` → se truncar, mostrar aviso (decisão 2 do plano).
- **Validação manual pendente** (após codar): round-trip dos 6 `transferType` + gate + caminhos-infelizes do picker.

**Armadilhas desta sessão (achados que ainda não viraram código):**
- **Times = REUSO total, zero fetch novo.** O picker `@team` já busca os times (`useTeams().teams` via `fetchStoreTeams`, [teams.ts:103](src/utils/teams.ts#L103)) — é o mesmo curl `Team?where=<retailer Pointer>` que o Andy passou. O picker "Por time → simples" só consome o `teams` do contexto.
- **Writer já pronto:** `updateActionFields` ([editIntent.ts:651](src/utils/editIntent.ts#L651)) já grava `transferType`+`value` (`value||null`). Apply via `updateActionFields(intent,'transfer',{transferType,value},ci)` — sem helper novo na Fase 2.
- `direct4userCurrent` ≠ `direct4userPrevious` (atual vs último humano). Fica **fora do menu**, só preservado no import como `<option>` extra (anti-corrupção, padrão captura/Loja física).
- Default do template muda de `direct4group` → `direct4userPrevious` (único sem campo → nó nasce válido).
- Molde a copiar: a feature "Loja física"/`@entity` (`fetchStoreEntities`/`entities` no contexto, `StoreActionSection` no painel) é o clone direto para a camada de vendedores.

**Próximo passo imediato:** criar `scripts/probe-users.mjs` (read-only, `POST {base}/parse/functions/getSupervisedUsersV2` body `{offset:0,limit:100,search:".*"}`, token via `flow-viewer.env`/`OMNI_TOKEN`, NÃO logar token) para confirmar base (`api-private` vs `api-private2`) + shape; depois `src/utils/users.ts` (clone de [entities.ts](src/utils/entities.ts)) + `users.test.ts`.

**Ponteiros:**
- Plano completo: PLANS.md § "Feature — Nó Transferência rico" (topo) — tabela dos 6 tipos, decisões 1–6, fases, riscos.
- Nó atual (a reescrever): bloco `kind === 'transferNode'` em [DetailPanel.tsx:3054](src/components/DetailPanel.tsx#L3054); `TRANSFER_TYPES` em [DetailPanel.tsx:83](src/components/DetailPanel.tsx#L83).
- Molde (Loja física): [entities.ts](src/utils/entities.ts), `StoreActionSection`/estados de picker em [DetailPanel.tsx](src/components/DetailPanel.tsx), contexto em [TeamsContext.tsx](src/contexts/TeamsContext.tsx)/[App.tsx](src/App.tsx).
- Times (reuso): [teams.ts:103](src/utils/teams.ts#L103) (`fetchTeams`/`fetchStoreTeams`).

**Skills sugeridas:** `/code-review` antes de commitar; `/verify` (ou `/run`) para validar o round-trip dos 6 tipos + pickers com token real.
<!-- HANDOFF:END -->
