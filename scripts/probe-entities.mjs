/**
 * Sonda READ-ONLY do endpoint de Listas (entities) — diagnóstico.
 * Não escreve nada. Lê o token de OMNI_TOKEN ou de flow-viewer.env.
 * Uso: node scripts/probe-entities.mjs <botId>
...
if (!botId) { console.error('Uso: node scripts/probe-entities.mjs <botId>'); process.exit(1) }
if (!token) { console.error('Sem token (OMNI_TOKEN ou flow-viewer.env).'); process.exit(1) }

const url = `${API}/v1/${botId}/entities`
console.log(`GET ${url}`)
...
if (list) {
  ...
  for (const e of list) console.log(`  - type=${JSON.stringify(e.type)} name=${JSON.stringify(e.name)} id=${e.id ? 'ok' : 'AUSENTE'}`)
  console.log(`tipo 'store': ${list.filter(e => e.type === 'store').length}`)
}
