...
### Adicionado
- **Remover conexões pelo botão "×" na aresta** — toda aresta de fluxo interna (`-next` e de escolha) ganhou um botão **×** no meio para remover a conexão direto no canvas, uma forma descobrível além do atalho Delete (que segue valendo). Implementado como aresta customizada `DeletableEdge` ([src/components/edges/DeletableEdge.tsx](src/components/edges/DeletableEdge.tsx)) registrada em `edgeTypes`; o clique cai no **mesmo caminho** da exclusão por teclado (`handleEdgesChange` → `applyEdgeDelete` + histórico/undo). Arestas para **outro bot** (`-ext`) e de **contexto** seguem sem botão (não são removíveis aqui). O `parseFlow` marca só as arestas internas com `type: 'deletable'`. _Remover uma conexão `-next` reseta o `next` para a forma canônica sem destino; remover uma de escolha esvazia o slot mantendo o botão._
